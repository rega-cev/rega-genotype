 ...
 */
public class SelfScanWidget extends Template {
		...
		runB.clicked().addListener(runB, new Signal.Listener() {
			public void trigger() {
				...
				Thread t = new Thread(new Runnable() {
					public void run() {
						AlignmentAnalyses blastAnalysis = BlastFileEditor.readBlastXml(
						...
						for (Cluster c: blastAnalysis.getAllClusters()) {
							...
							if (new File(analysisFile).exists()) {
								...
								for (String ws: windowSizes) {
									...
									String traceFile = workDir.getAbsolutePath() 
											+ File.separator + traceFileName(ws);
									...
									try {
										genotypeTool.analyzeSelf(traceFile, 
												analysisFile, windowSize, stepSize, analysisId);
									...
									for (Element phyloMajorElement: phyloMajorResults)
										if (phyloMajorElement.getAttributeValue("id").endsWith("self-scan")) {
											String scanResult = "genotype_result/result[@id='" + 
													phyloMajorElement.getAttributeValue("id") + "']";
											if (parser.elementExists(scanResult)) {
												...
												reportC.addWidget(new WText(
														"<div>Self scan: " + phyloMajorElement.getAttributeValue("id") + "</div>"));
			...
			private String traceFileName(String windowSize) {
				return ToolVerificationWidget.SELF_SCAN_RESULT_FILE + windowSize + ".xml";
			}
