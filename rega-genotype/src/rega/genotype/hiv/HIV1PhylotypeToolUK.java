...
public class HIV1PhylotypeToolUK extends GenotypeTool {
    ...
    public HIV1PhylotypeToolUK() throws IOException, ParameterProblemException, FileFormatException {
        hiv1 = readAnalyses("hivphylotypesUK.xml");
        ...
        if (s.getLength() > 800) {
            ...
            if (crfResult.haveSupport()
                    && (crfResult.getSupportInner() > crfResult.getSupportOuter())) {
            			// Rule Cluster
            			conclude(crfResult,
            			"Cluster assigned based on sequence > 800 bps " +
            			"grouping with a defined HIV-1 cluster with bootstrap > 70% " +
            			"and inside bootstrap support higher than outside bootstrap support");  
          } else if (scanResult.haveSupport()) {
